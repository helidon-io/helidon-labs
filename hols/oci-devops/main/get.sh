...
get_compartment_id() {
  ...
  compartment_id=$(jq -r '.resources[] | select(.type == "oci_core_instance").instances[].attributes | select(.display_name == "instance-helidon-demo") | .compartment_id' "${TERRAFORM_TFSTATE}")
...
create_ssh_private_key() {
  rm -rf private.key
  local private_key
  private_key=$(jq -r '.outputs.generated_ssh_private_key.value' "${TERRAFORM_TFSTATE}")
